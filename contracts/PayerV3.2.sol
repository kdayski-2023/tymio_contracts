...
contract PayerV3Draft {    
    ...
    modifier onlyOwnerOrService() {
        require(    msg.sender == owner1 ||
                msg.sender == owner2 ||
                msg.sender == service,  "NOT THE ALLOWED ADDRESS"
        );
        _;
    }
    ISwapRouter public swapRouter = ISwapRouter(0xE592427A0AEce92De3Edee1F18E0157C05861564);
    uint24 poolFee = 500; 
    uint24 maxAdditionalAmountPercentage  = 200;
    ...
    function depositAndOrder(IERC20 _tokenAddressIn, IERC20 _tokenAddressOut, uint256 _amount, uint256 _price, uint256 _duration)public{
        deposit(_tokenAddressIn, _amount);
        makeOrder(_tokenAddressIn,_tokenAddressOut, _amount, _price, _duration);
    }

    
    function depositEthAndOrder(IERC20 _tokenAddressIn, IERC20 _tokenAddressOut, uint256 _amount, uint256 _price, uint256 _duration)public payable{
    ...
    function makeOrder(IERC20 _tokenAddressIn, IERC20 _tokenAddressOut, uint256 _amountIn, uint256 _price, uint256 _duration) public {
    ...
    function deposit(IERC20 _tokenAddress, uint256 _amount) public payable {
    ...
    function calculatePercentage(uint256 _quantity, uint256 _percentage) public pure returns (uint256) {
        return _quantity.mul(_percentage).div(10000); // ??? maybe 100
    }
    
    
    function calculateProportion(uint256 _quantity, uint256 _total, uint256 _accuracy) public pure returns (uint256) {
        return _quantity.mul(_accuracy).div(_total);
    }

    function claimOrder(uint256 _orderId, address _usdToken, bool _force) public {
    ...
    function fullWithdrawal(IERC20 _tokenAddress, uint256 _amount) public {
    ...
    function balanceOf(IERC20 _tokenAddress, address _user) public view returns (uint256) {
        return balances[_tokenAddress] [_user] ; // ??? Can throw call of null if balances[_tokenAddress]==null
    }

    
    function _balanceTransfer(IERC20 _tokenAddress, address _sender, address _recipient, uint256 _amount) internal {
        // ??? check balances[_tokenAddress] is NOT NULL
    ...
    function getBackEth(address payable _to, uint256 _amount) external payable onlyOwners {
        (bool sent, ) = _to.call{value: _amount}("");
        require(sent, "Failed to send Ether");
    }

    
    function emergencyQuit(address _user, IERC20  _tokenAddress, uint256 _amount) external onlyOwners {
